...
def finetune(args):
    ...
    facenet = facenet.to(args.device)
    tf_gray = transforms.Grayscale(num_output_channels=3)
    target_ =[]
    for i in range(targets.shape[0]):     
        target_.append(facenet(tf_gray(targets[i,:,:,:]).unsqueeze(0).to(args.device)))
    target_ = torch.stack(target_, 0)
    ###
    ...
    for idx in tqdm(range(args.num_iter)):
        ...
        img_ =[]
        for i in range(targets.shape[0]):     
            img_.append(facenet(tf_gray(img[i,:,:,:]).unsqueeze(0).to(args.device)))
        img_ = torch.stack(img_, 0)
        id_loss = F.cosine_similarity(img_,target_)
        ###
