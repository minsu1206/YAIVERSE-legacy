...
def unit_test(args):
    ...
    if args.unit_test == 0:
        ...
        test_name = 'Load Face Detection / Landmark model'
    elif args.unit_test == 3:
        test_name = 'Forward : Face Align'
    ...
    if args.unit_test >= 4:
        ...
        # GPU warming up
        # dummy = Image.fromarray(np.uint8(np.zeros((1024, 1024, 3))))

        # start = time.time()
        # _ = e4e_projection(dummy, name=img_path.replace('image.png', 'dummy.pt'), net=inversion_net, save_inverted=False, device=device)
        # print("CUDNN benchmark - warming up : ", time.time() - start)
        ...
        for _ in tqdm(range(test_iter)):
            ...
            # print("my_w .shape : ", my_w.shape)    # (18, 512)
